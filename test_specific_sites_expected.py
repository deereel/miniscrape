...
for i, (expected_site, exp_first, exp_last, exp_company, exp_address) in enumerate(expected):
    ...
    match = "✅" if exp_company.lower() in actual_company.lower() or actual_company.lower() in exp_company.lower() else "❌"
...
for i, (expected_site, exp_first, exp_last, exp_company, exp_address) in enumerate(expected):
    ...
    match = "✅" if expected_officer.lower() in actual_officer.lower() or actual_officer.lower() in expected_officer.lower() else "❌"
...
# Save results to CSV
with open("test_specific_sites_results.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
...
print("\nResults saved to test_specific_sites_results.csv")
